...
- **Nginx 全局 Gzip 开启**：解开 `gzip_types`（含 `application/json`、`text/css`、`application/javascript`、`image/svg+xml`）并启用 `gzip_vary on`
  ...
  > 上表为 Gzip 开启当次的实测快照。当前线上复测：`data.json` 132,502 B → 8,844 B（-93.3%）、`img/body-outline.svg` 69,039 B → 28,118 B（-59.3%）、`css/fitness.css` 33,987 B → 6,843 B（-79.9%）、`index.html` 58,454 B → 13,279 B（-77.3%）。
